...
const ProductSingleDetails: React.FC<ProductSingleDetailsProps> = ({
  ...
  const { price, basePrice, discount } = usePrice(
    product && {
      amount: product.attributes["sale-price"] ?? product.attributes.price,
      baseAmount: product.attributes.price,
      currencyCode: "USD",
    }
  );
    ...
    <div className="block lg:grid grid-cols-9 gap-x-10 xl:gap-x-14 pt-7 pb-10 lg:pb-14 2xl:pb-20 items-start">
      {width < 1025 ? (
          ...
          {dummyGallery.map((item, index) => (
            <SwiperSlide key={`product-gallery-key-${index}`}>
              <div className="col-span-1 transition duration-150 ease-in hover:opacity-90">
                <img
                  src={
                    item ?? "/assets/placeholder/products/product-gallery.svg"
                  }
                  alt={`${product?.attributes.name}--${index}`}
        ...
        <div className="col-span-5 grid grid-cols-2 gap-2.5">
          {/* @todo Add galery */}
          {dummyGallery.map((item, index: number) => (
            <div
              key={index}
      ...
      <div className="col-span-4 pt-8 lg:pt-0">
        ...
        <div className="flex items-center space-s-4 md:pe-32 lg:pe-12 2xl:pe-32 3xl:pe-48 border-b border-gray-300 py-8">
          ...
          >
            <span className="py-2 3xl:px-8">{t("text-add-to-cart")} (WIP)</span>
          </Button>
        </div>
        <div className="py-6">
        ...
        {/* TODO: No review data yet */}
        {/* <ProductMetaReview product={product} /> */}
      </div>
    </div>
  );
};
